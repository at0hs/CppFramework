...
const MessageTask<Commands>::EventMap kEvents{ { Commands::TestCommand1, { test_handler } },
											   { Commands::TestCommand2, { test_handler } } };

const MessageTask<Commands>::EventMap kRpcEvents{ { Commands::TestCommand1, { rpc_test_handler } },
												  { Commands::TestCommand2,
													{ rpc_test_handler } } };

const MessageTask<Commands>::EventMap kExceptionEvents{
	{ Commands::TestCommand1, { throw_exception } }, { Commands::TestCommand2, { throw_exception } }
};
...
TEST_F(MessageTaskTest, SendEventWithEventMapInitialization) {
	...
	EXPECT_EQ(Commands::TestCommand1, last_request.get_command());
	EXPECT_STREQ(command1_payload.c_str(), last_request.get_payload_as<std::string>().c_str());
	...
	EXPECT_EQ("Test", last_request.get_from());
	EXPECT_EQ(Commands::TestCommand2, last_request.get_command());
	EXPECT_STREQ(command2_payload.c_str(), last_request.get_payload_as<std::string>().c_str());
}

TEST_F(MessageTaskTest, SendEventWithInitializerList) {
	std::string command1_payload = "Command1";
	std::string command2_payload = "Command2";
	MessageTask<Commands> task("TestTask", { { Commands::TestCommand1, { test_handler } },
											 { Commands::TestCommand2, { test_handler } } });
...
TEST_F(MessageTaskTest, RpcEvent) {
	...
	EXPECT_TRUE(task.rpc_event({ "Test", Commands::TestCommand1, command1_payload }));
	EXPECT_EQ("Test", last_request.get_from());
	EXPECT_EQ(Commands::TestCommand1, last_request.get_command());
	EXPECT_STREQ(command1_payload.c_str(), last_request.get_payload_as<std::string>().c_str());
...
TEST_F(MessageTaskTest, RpcEventTimeout) {
	...
	EXPECT_FALSE(task.rpc_event({ "Test", Commands::TestCommand1, command1_payload },
...
TEST_F(MessageTaskTest, ExceptionInHandler) {
	...
	try {
		task.rpc_event({ "Test", Commands::TestCommand1, command1_payload });
